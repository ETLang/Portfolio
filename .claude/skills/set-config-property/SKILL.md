...
| `simulation.<key>` | number | One of the 5 simulation tunables - see [simulation_tunables_panel.ts](../../../src/simulation_tunables_panel.ts) (`raysPerFrame`, `integrationInterval`, `photonBounces`, `surfaceBias`, `ambientScatterSoftness`). |
| `simulation.width` / `simulation.height` | number | Live-resizes the simulation resolution via `LitboxSceneRenderer.resizeSimulation` (an async full scene rebuild, unlike the plain-property tunables above - expect this call to take noticeably longer). The dimension you don't specify is carried over from the simulation's current effective resolution. Resetting this also resets denoiserTunables/simulationTunables to the active scene's defaults, same as switching scenes. |
